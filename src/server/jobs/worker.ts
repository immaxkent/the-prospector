...
export type JobHandler = (db: Database, payload: Record<string, unknown>, now: Date) => Promise<void>;

export const JOB_HANDLERS: Record<string, JobHandler> = {
  [DAILY_RUN_JOB]: async (db, payload, now) => {
...
export async function runJob(db: Database, job: JobRow, now: Date) {
  const handler = JOB_HANDLERS[job.type];
  if (!handler) throw new Error(`no handler for job type ${job.type}`);
  await handler(db, job.payload, now);
}
...
export async function tick(
  db: Database,
  opts: { workerId: string; now?: Date; scheduleHour: number; max?: number },
): Promise<TickResult> {
...
export async function startWorker(db: Database, opts: WorkerOptions) {
  ...
  while (!opts.signal?.aborted) {
    try {
      const result = await tick(db, { workerId: opts.workerId, scheduleHour: opts.scheduleHour });
      opts.onTick?.(result);
    } catch (err) {
